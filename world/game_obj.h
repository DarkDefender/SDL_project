...
class GameObj {
  ...
  vector<Texture> load_mat_tex(aiMaterial *mat, aiTextureType type,
                               string typeName);
  void clean_up();
...
public:
  ...
  GameObj(string mdl_path, string shader_name, string type = "GameObj",
          Btype b_type = NONE, btVector3 pos = btVector3(0, 0, 0),
          btQuaternion quat = btQuaternion(0, 0, 0));
  GameObj(string mdl_path, string col_path, string shader_name,
          string type = "GameObj", Btype b_type = NONE,
