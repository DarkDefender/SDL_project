...
void Camera::set_pos(btVector3 pos) { cur_pos = pos; }
...
void Camera::get_pos(btVector3 &pos) { pos = cur_pos; }
...
void Camera::get_view_dir(btVector3 &dir) { dir = view_dir; }
...
void Camera::get_rot_mat(btMatrix3x3 &mat) {
  ...
  mat = btMatrix3x3(xaxis.x(), yaxis.x(), view_dir.x(), xaxis.y(), yaxis.y(),
                    view_dir.y(), xaxis.z(), yaxis.z(), view_dir.z());
}
...
void Camera::move(btVector3 delta) { set_pos(cur_pos + delta); }
...
void Camera::set_track_point(btVector3 pos) { track_point = pos; }
void Camera::set_track_point(GLfloat x, GLfloat y, GLfloat z) {
  set_track_point(btVector3(x, y, z));
}

void Camera::set_track_obj(btRigidBody *obj) { track_obj = obj; }
void Camera::set_follow_obj(btRigidBody *obj) {
  follow_obj = obj;
  update_timer.stop();
}

void Camera::set_follow_offset(btVector3 off) { follow_offset = off; }
...
void Camera::look_at(btVector3 l) {
  ...
  btTransform rot;
  rot.setIdentity();
  rot.setBasis(btMatrix3x3(u.x(), u.y(), u.z(), v.x(), v.y(), v.z(), n.x(),
                           n.y(), n.z()));
  btTransform trans2;
  trans2.setIdentity();
  trans2.setOrigin(-p);
...
void Camera::add_waypoint(btVector3 pos) { waypoints.push_back(pos); }
void Camera::add_waypoint(GLfloat x, GLfloat y, GLfloat z) {
  add_waypoint(btVector3(x, y, z));
}

void Camera::set_manual(bool man) { manual = man; }
...
void Camera::update() {
  ...
  if (waypoints.size() > 0) {
    ...
    if (dist > move_dist) {
      ...
      move((waypoints[(waypoint + 1) % waypoints.size()] - waypoints[waypoint])
               .normalized() *
           (move_dist - dist));
      waypoint++;
...
void Camera::OGL_mat(GLfloat *m) { trans.getOpenGLMatrix(m); }
