...
ShipObj::ShipObj(string mdl_path, string shader_name, btVector3 pos,
                 btQuaternion quat)
...
ShipObj::ShipObj(string mdl_path, string col_path, string shader_name,
...
void ShipObj::update() {
  ...
  if (sim_timer.delta_s() > 0.001f && false) {
    ...
    btVector3 x_vec =
        btVector3(0, 1, 0).cross(trans.getBasis().getColumn(2)).normalized();
    ...
    if (x_vec_scale < 0.95f) {
      ...
      if (dir > 0) {
        get_body()->applyTorque(
            btVector3(0, (1 - x_vec_scale) * 100.0f * sim_timer.delta_s(), 0));
      } else {
        get_body()->applyTorque(
            btVector3(0, (-1 + x_vec_scale) * 100.0f * sim_timer.delta_s(), 0));
      }
    ...
    if (!turn_timer.isStarted() && x_vec_scale > 0.8f) {
      if (dir > 0) {
        get_body()->applyTorque(
            trans.getBasis() *
            btVector3(0, 0, (1 - x_vec_scale) * 200.0f * sim_timer.delta_s()));
      } else {
        get_body()->applyTorque(
            trans.getBasis() *
            btVector3(0, 0, (-1 + x_vec_scale) * 200.0f * sim_timer.delta_s()));
  ...
  if (trail_timer.delta_s() * cur_speed > 0.1f) {
    ...
    GameObj *trail1 =
        new TrailObj("../res/trail.obj", "fullb",
                     trans.getOrigin() + spawn_x - spawn_z, travel_dir * 0);
    GameObj *trail2 =
        new TrailObj("../res/trail.obj", "fullb",
                     trans.getOrigin() - spawn_x - spawn_z, travel_dir * 0);
  ...
  if (shooting && shoot_timer.delta_s() > 0.1f) {
    ...
    spawn_new_obj("las_shoot",
                  trans.getOrigin() + travel_dir +
                      0.4 * trans.getBasis().getColumn(0).normalized(),
                  travel_dir, this);
    spawn_new_obj("las_shoot",
                  trans.getOrigin() + travel_dir +
                      -0.4 * trans.getBasis().getColumn(0).normalized(),
