...
GameObj::GameObj(string mdl_path, string shader_name, string type, Btype b_type,
                 btVector3 pos, btQuaternion quat) {
...
GameObj::GameObj(string mdl_path, string col_path, string shader_name,
                 string type, Btype b_type, btVector3 pos, btQuaternion quat) {
  ...
  if (obj_coll_shape.count(col_path) == 0) {
    if (meshes.size() == 1) {
      obj_coll_shape[col_path] =
          new btConvexHullShape(&(meshes[0]->vertices[0].Position.x),
      ...
      for (uint32_t i = 0; i < meshes.size(); i++) {
        btConvexHullShape *hull =
            new btConvexHullShape(&(meshes[i]->vertices[0].Position.x),
        ...
        btTransform trans;
        // No offsets. The shapes should be (in this case) corretly setup from
        // the start
        trans.setIdentity();
...
void GameObj::init() {
  ...
  btDefaultMotionState *MotionState =
      new btDefaultMotionState(btTransform(spawn_quat, spawn_pos));
  btScalar mass = 10;
  ...
  // Add this GameObject to the bullet for when we do collision detection
  // (health etc)
  ...
  // Don't simulate collisions with this object only use it to check if a
  // collision has occured
  phys_body->setCollisionFlags(phys_body->getCollisionFlags() |
...
GameObj::~GameObj() {
  ...
  // Clean up new object spawn list if we for some reason didn't manage to empty
  // if before
...
Mesh *GameObj::process_mesh(aiMesh *mesh, const aiScene *scene) {
  ...
  if (mesh->mMaterialIndex >= 0) {
    ...
    vector<Texture> diffuseMaps =
        load_mat_tex(material, aiTextureType_DIFFUSE, "texture_diffuse");
    textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());

    vector<Texture> specularMaps =
        load_mat_tex(material, aiTextureType_SPECULAR, "texture_specular");
...
vector<Texture> GameObj::load_mat_tex(aiMaterial *mat, aiTextureType type,
                                      string typeName) {
  vector<Texture> textures;
...
btRigidBody *GameObj::get_body() { return phys_body; }

btDiscreteDynamicsWorld *GameObj::get_world() { return phys_world; }

void GameObj::set_dead(bool dead) { this->dead = dead; }

bool GameObj::get_dead() { return dead; }

int GameObj::get_hp() { return health; }

void GameObj::set_hp(int hp) { health = hp; }

void GameObj::apply_dmg(int dmg) { health -= dmg; }

void GameObj::set_render_scale(float scale) { render_scale = scale; }

GameObj *GameObj::get_spawn_obj() { return spawn_obj; }

void GameObj::set_spawn_obj(GameObj *obj) { spawn_obj = obj; }
...
void GameObj::render() {
  ...
  if (camera != NULL && b_type != NONE) {
    ...
    btMatrix3x3 mat =
        btMatrix3x3(x_axis.x(), y_axis.x(), z_axis.x(), x_axis.y(), y_axis.y(),
                    z_axis.y(), x_axis.z(), y_axis.z(), z_axis.z());
    trans.setBasis(mat);
  }

  if (render_scale != 1) {
    trans.setBasis(trans.getBasis().scaled(
        btVector3(render_scale, render_scale, render_scale)));
  }
...
void GameObj::spawn_new_obj(string type, btVector3 pos, btVector3 trav_dir,
  ...
  if (type == "las_shoot") {
    ...
    btMatrix3x3 mat =
        btMatrix3x3(xaxis.x(), trav_dir.x(), zaxis.x(), xaxis.y(), trav_dir.y(),
                    zaxis.y(), xaxis.z(), trav_dir.z(), zaxis.z());

    btTransform temp_trans(mat);

    new_objs.push_back(new GameObj("../res/laser_shot1.obj", "fullb", "GameObj",
                                   Y_AXIS, pos, temp_trans.getRotation()));
...
void GameObj::spawn_new_obj(GameObj *obj) { new_objs.push_back(obj); }
...
void GameObj::set_camera(Camera *cam) { camera = cam; }
